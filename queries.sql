...
-- 2. All purchases of a specific customer (id = 1) with product details and cost
SELECT c.lname,
       c.fname,
       c.phone,
       oi.quantity,
...
-- 3. Top 10 products with the highest sales quantity
...
-- 8. Shipment information for each order
...
-- 9. Total sales per month (for year 2024)
...
-- 12. Products with average cost per item (for ordered products)
SELECT p.id_product, p.name, p.price AS avg_price
FROM product p
JOIN itemsordered oi ON p.id_product = oi.id_product
GROUP BY p.id_product;

-- 13. Customers who paid using a specific payment method (e.g., Credit Card)
...
-- 14. Products ordered at least 5 times
...
-- 15. Customers who purchased the highest number of distinct products
